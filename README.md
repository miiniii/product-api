...
- 재시도는 락을 얻기 전까지 여러 번 시도하며 대기하므로, 실제 처리되는 요청 수(TPS)는 줄어듦(전체 요청 수 대비 성공 비율 낮음)
- 락 대기 중인 요청들이 계속 tryLock() 반복 → CPU, 네트워크, Redis에 부하 증가

- 락을 못 잡은 요청은 계속 실패 → 재시도 반복 → 실패 → 재시도 .. 반복
- MAX_RETRIES 설정(3회)에 도달하면 결국 예외 발생 → 에러율 증가

- retry로 인해 중복 시도 → 하나의 요청이 여러 번 락 시도 → TPS 하락
- 실패 요청 증가 → 락 획득 실패 + 재시도 후 실패 (→ 예외) → 에러율 증가
- 재시도 후 성공한 요청은 응답 시간이 낮음 -> 평균 응답시간 낮게 나옴
