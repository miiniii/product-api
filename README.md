...
- 병렬성이 올리간다고 TPS가 무조건 높아지는 건 아님 대신 락 충돌 상황에서 재시도/실패가 늘어난 것으로 판단
- Redis 분산 락 충돌에 따른 대기 시간 증가로 분석 → 락 획득 대기시간(waitTime)이 응답시간 증가에 직접적인 영향을 준 것

| 구성 환경 | TPS (평균) | 응답시간 평균 (ms) | 응답시간 최소 (ms) | 응답시간 최대 (ms) | 에러율 (%) |
|----------|------------|--------------------|--------------------|--------------------|------------|
| Retry O  | 111.0      | 217.69             | 71                 | 123.5              | 16.85%     |
| Retry X  | 23.0       | 38.93              | 16.5               | 27                 | 37.69%     |

retry를 넣었는데 TPS가 떨어지고 에러율 증가 이유

- 재시도는 락을 얻기 전까지 여러 번 시도하며 대기하므로, 실제 처리되는 요청 수(TPS)는 줄어듦
- 락 대기 중인 요청들이 계속 tryLock() 반복 → CPU, 네트워크, Redis에 부하 증가

락을 더 오래 점유하게 됨

- leaseTime이 길면 한번 락을 잡은 요청이 그 시간만큼 자리를 차지
- 그 사이에 다른 요청은 계속 실패 → 재시도 반복 → 실패 → 재시도 .. 무한 반복되는 상황
- MAX_RETRIES 설정에 도달하면 결국 예외 발생 → 에러율 증가

- retry로 인해 중복 시도 → 하나의 요청이 여러 번 락 시도 → TPS 하락
