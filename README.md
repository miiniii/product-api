...
|- 특정 인스턴스에 집중적으로 몰려서 락 충돌 발생한것으로 판단 <br> - 충돌이 몰리면서 재시도 전에 실패하거나 타임 아웃 <br> - 실패가 많아 에러율 높음 |- 요청 수가 더 많아지고, 락 점유 시간이 누적되어 <br>1. 하나의 트랙잭션이 row에 락을 걸고 있는 동안, 다른 트랙잭션이 대기 상태로 들어감 <br>2. 이때부터 InnoDB는 내부적으로 row_lock_wait 카운트하기 시작 <br>3. 락을 가진 트랜잭션은 아직 커밋 안했고, 다른 트랜잭션은 같은 row에 접근 시도 → 대기 발생 → wait 수치 증가 <br> - 락이 완전히 풀리기 전까지는 다른 트랜잭션이 진입 불가 <br>- 동시 유저 수가 많을 수록 이런 병목 구간이 폭발적으로 증가 <br>|
