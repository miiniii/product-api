...
|- 특정 인스턴스에 집중적으로 몰려서 락 충돌 발생한것으로 판단 <br> - 충돌이 몰리면서 재시도 전에 실패하거나 타임 아웃 <br> - 실패가 많아 에러율 높음 | - 요청이 여러 인스턴스로 분산되어 일부만 락 충돌 <br>
- 분산된 요청이 재시도 기회를 갖고 처리에 성공 <br> - 재시도 후 처리 성공 → 에러율 낮음 <br> |
